...
namespace ResumeAPI.Endpoints
{
	class PersonEndpoints
	{
		public static void RegisterEndpoints(WebApplication app)
		{
			...
			app.MapPost("/person", async (PersonService personService, PersonDTO personDTO) =>
			{
			...
			app.MapPut("/person/{id}", async (PersonService personService, int id, PersonDTO personDTO) =>
			{
				var validationContext = new ValidationContext(personDTO);
				var validationResult = new List<ValidationResult>();
